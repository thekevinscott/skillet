...
def describe_tune():
    ...
    async def it_runs_tuning_rounds_until_target(skillet_env: Path, mock_llm_calls):
        ...
        with (
            ...
            config = TuneConfig(
                max_rounds=3, target_pass_rate=100.0, samples=1, parallel=1
            )
    ...
    async def it_stops_after_max_rounds(skillet_env: Path, mock_llm_calls):
        ...
        with (
            ...
            config = TuneConfig(
                max_rounds=2, target_pass_rate=100.0, samples=1, parallel=1
            )
    ...
    async def it_calls_callbacks_during_tuning(skillet_env: Path, mock_llm_calls):
        ...
        with (
            ...
            config = TuneConfig(
                max_rounds=1, target_pass_rate=100.0, samples=1, parallel=1
            )
