...
### Changed
- CI: the `pr-monitor` gate workflow gains a `concurrency:` group with `cancel-in-progress`. A gate job resolves its target SHA at startup, so pushing a new commit left the old run polling for runs on a superseded commit and holding a runner until it finished. Keyed on `github.ref` (`refs/pull/N/merge` for a `pull_request` event), so it scopes per PR
...
- CI: the `pr-monitor` gate job now declares the permissions the action actually uses (`actions: read` to list workflow runs, `contents: read`, `pull-requests: read` for willfire's prediction). The previous `permissions: checks: read` block set every unlisted scope to `none`, which is latent while this repo is public — a starved token still reads public data — and an outage the day it is not. Adds `timeout-minutes: 20`: the action has no timeout of its own, so the caller's job timeout is the only backstop, and without it a predicted run that never dispatches would hang until GitHub's 6-hour limit
...
- Internal bespoke cache machinery — `cache_lock` (filelock-based), `get_cached_iterations`, `save_iteration`, and `get_cache_dir` — superseded by cachetta's decorator

### Changed
- `claude_agent_sdk` is now an optional extra (`skillet[tune]`); skillet no longer requires Claude to run evals. The eval path (run + judge + failure summary) and `generate-evals` route structured output through the selected `--agent`'s CLI, and the SDK is imported lazily only by the parked `tune`/DSPy path — a fresh install without the extra can run `skillet eval --agent codex` end-to-end, while `tune` raises a clear "install `skillet[tune]`" error when the SDK is absent
- Eval result caching now goes through cachetta's decorator (keyed by eval + agent + skill + iteration) instead of the bespoke count-based file layout, per-directory file lock, and manual glob/read/write — cachetta owns reads, atomic writes, and in-flight de-duplication. The cache is constructed at runtime in `evaluate()` and threaded down, rather than bound from a `config.CACHE_DIR` global at import time. **Behavior change:** a sparse cache (e.g. iterations 1 and 3 present, 2 missing) now re-runs the specific missing iteration instead of mis-reading the third file as iteration 2
