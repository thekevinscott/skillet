...
### Added
- Run-time harness selection for the agent under test: `skillet eval --harness codex` (or `evaluate(harness="codex")`) runs portable evals through [lite-harness](https://github.com/LiteLLM-Labs/lite-harness) instead of the Claude Agent SDK (default `claude`). The harness is a run-time choice — never stored in the eval file — so eval sets stay portable across harnesses; the `--harness` flag also falls back to the `SKILLET_HARNESS` env var so the invoking agent can set it. The judge always stays on the Claude Agent SDK for score comparability, and cache keys are namespaced by harness so Claude and Codex runs of the same eval don't collide. Adding a connector is a one-line registry entry. lite-harness is an optional, lazily-imported dependency (preview-stage, not yet on PyPI) — see [CLI reference](docs/reference/cli.md#harness) for setup
