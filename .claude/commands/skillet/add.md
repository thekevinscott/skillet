...
2. **Check existing evals** — List directories in `~/.skillet/evals/` to see what eval categories already exist.
...
4. **Save the eval** — Write a YAML file to `~/.skillet/evals/<name>/<number>.yaml` containing:
...
You: *checks ~/.skillet/evals/ and finds browser-fallback/ with 2 evals*
...
You: *saves to ~/.skillet/evals/browser-fallback/003.yaml*
"Eval saved to ~/.skillet/evals/browser-fallback/003.yaml
...
- Create the ~/.skillet/evals directory if it doesn't exist
